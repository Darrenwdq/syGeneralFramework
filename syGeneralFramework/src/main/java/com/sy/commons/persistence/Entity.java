package com.sy.commons.persistence;
...
import com.sy.commons.persistence.dao.impl.BaseDomain;
...
public abstract class Entity extends BaseDomain implements Comparable<Entity> {
	...
	public String identityString() {
		...
		for (Field field : ObjectUtil.getAllField(clazz)) {
			...
			if (id != null) {
				try {
					Object value = new PropertyDescriptor(field.getName(), clazz).getReadMethod().invoke(this);
	 ...
	 * @param other
	 *            比较的对象
	 * @return List<String> 属性值不同的列集合 创建日期：2012-11-21 修改说明：
	 * @author wangk
	 */
	 ...
	 *
	 * @param <T>
	 *            属性类型参数，必须用属性的实际类型及其父类型接收返回值，否则将抛出类型转换异常
	 *            指定的列为空或不存在时将抛出RuntimeException
	 * @param columnName
	 *            列名
	 * @return T 属性值 创建日期：2012-11-21 修改说明：
	 * @author wangk
	 */
